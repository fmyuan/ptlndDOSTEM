...
void Richard::update(Layer * frontl, Layer *backl, Layer *fstsoill,
	...
	//Yuan: calculation
	iterate(fstsoill, trans, evap, infil, drain);
	...
	while (currl != NULL) {
		...
		if (ind > numal) {
			break;
			//    		liqld[ind] = currl->liq;	//Yuan: already break, then why this ?
		}
...
void Richard::iterate(Layer *fstsoill, const double & trans,
	...
	while (tmld < 1) {
		...
		int st = updateOneTimeStep(fstsoill, trans, evap, infil, drain);
		if (st == -1) {
...
int Richard::updateOneIteration(Layer *fstsoill, const double & trans,
	...
	/*
	for (int ill = 0; ill < numal; ill++) {
		cout<<ill<<"\t"<<liqii[ill]<<"\t";
		if (liqii[ill] > iithres)
			totalfrac = totalfrac + rootfr[ill];
		cout<<rootfr[ill]<<"\t"<<totalfrac<<endl;
	}
*/
	...
	//if (liqii[ind] > iithres)
	//	rmx[ind] = qin[ind] - qout[ind] - qtrans * rootfr[ind] / totalfrac;
	//else
	//	rmx[ind] = qin[ind] - qout[ind];
	...
	if (numal >= 2) {
		...
		//			- drain;
		//else
		//	rmx[ind] = qin[ind] - qout[ind] - drain;

		rmx[ind] = qin[ind] - qout[ind] - qtrans * rootfr[ind] - drain;
		amx[ind] = -dqidw0;
		bmx[ind] = dzmm[ind] / dt - dqidw1 + dqodw1;
		cmx[ind] = 0.;
	}
