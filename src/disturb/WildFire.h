...
class WildFire{
	public:
    	...
    	void prepareDrivingData(const bool runsp, const bool runtr);
	...
	private:
    	...
    	//Yuan: the following 4 variables
     	int onesize;
     	int oneseason;
     	double onevegseverity;
     	double onesoiseverity;
...
//Yuan: the following if using years will result in huge memory needs, if spin-up is long
		...
		double vegseverity[MAX_FIR_OCR_NUM];
		double soiseverity[MAX_FIR_OCR_NUM];
