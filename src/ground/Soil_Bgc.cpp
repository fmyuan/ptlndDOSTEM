...
void Soil_Bgc::CH4Flux(const int mind, const int id) {
	...
	double SS, torty, torty_tmp, diff_tmp, tmp_flux, Flux2A = 0.0, Flux2A_m =
			0.0;
	double Prod, Ebul, Oxid, Plant;
	double Ebul_m, Plant_m, totFlux_m, Oxid_m;
	double kfastc, kslowc, klitrc, TResp;//, MResp;
	double totPlant = 0.0, totEbul = 0.0, totPlant_m = 0.0, totEbul_m = 0.0,
			totOxid_m = 0.0;
	...
	ed->d_vegs.realLAI = ed->d_vegs.realLAI + (ed->d_vegs.currLAI
			- ed->d_vegs.preLAI) / 30.0;
	...
	for (il = 0; il < numsl; il++) {
		...
		//if (ed->m_soid.watertab > (ed->m_sois.z[il] + ed->m_sois.dz[il])) {
		if (ed->d_soid.watertab > 0.01) {
			torty_tmp = ed->m_sois.por[il] - ed->d_soid.alllwc[il]
					- ed->d_soid.alliwc[il];
			if (torty_tmp < 0.05)
				torty_tmp = 0.05;

			torty = 0.66 * torty_tmp * pow(torty_tmp / ed->m_sois.por[il], 3.0);
			diff_tmp = diff_a;

		} else {
			torty = 0.66 * ed->d_soid.alllwc[il] * pow(ed->d_soid.alllwc[il]
					/ (ed->m_sois.por[il] + ed->d_soid.alliwc[il]), 3.0);
	...
	for (j = 1; j <= m; j++) {
		...
		for (il = numsl - 1; il > 0; il--) {
			...
			//MResp1 = getRhmoist(ed->m_sois.por[il], bgcpar.moistmin,
			//		bgcpar.moistmax, bgcpar.moistopt);

			klitrc = bd->m_soid.kdl_m[il] * 0.5; //0.5
			kfastc = bd->m_soid.kdr_m[il] * 0.5;
			kslowc = bd->m_soid.kdn_m[il] * 0.5;


			Plant = bd->rp * ed->m_sois.rootfrac[il] * ed->d_sois.ch4[il]
					* bd->tveg * ed->d_vegs.realLAI / 2.0; //bd->m_vegd.lai / 2.0;

			if (ed->d_soid.watertab > (ed->m_sois.z[il] + ed->m_sois.dz[il]
					/ 2.0)) {
				...
				tmp_flux = ed->m_sois.por[il] - ed->d_soid.alllwc[il]
						- ed->d_soid.alliwc[il];
				if (tmp_flux < 0.05)
					tmp_flux = 0.05;

				Oxid = 5.0 * ed->d_sois.ch4[il] * TResp / (20.0
						+ ed->d_sois.ch4[il]);
				Oxid_m = Oxid * tmp_flux * ed->m_sois.dz[il] * 1000.0;
				...
				if (tmp_sois.nonc[il] > 0) {
					...
					del_soi2a.nrh_m[il] = (kfastc * (1.0 - rslowc) + kslowc
							* rslowc) * tmp_sois.nonc[il] * TResp;

				} else {
					del_soi2a.nrh_m[il] = 0.0;
				}

				Prod = 1000.0 * (del_soi2a.nrh_m[il] + del_soi2a.rrh_m[il])
						/ (ed->m_sois.dz[il] * ed->m_sois.por[il]) / 12.0;

				SB = 0.05708 - 0.001545 * ed->d_sois.ts[il] + 0.00002069
						* ed->d_sois.ts[il] * ed->d_sois.ts[il]; //volume

				Pressure = DENLIQ * G * (ed->m_sois.z[il] + ed->m_sois.dz[il]
						/ 2.0) + Pstd;
				...
				Ebul = (ed->d_sois.ch4[il] - SM) * 1.0;

				if (Ebul < 0.0)
					Ebul = 0.0;

				Ebul_m = Ebul * ed->d_soid.alllwc[il] * ed->m_sois.dz[il]
						* 1000.0;

				Oxid = 0.0;

				if (ed->d_sois.ts[il] > 0.0)
					Plant_m = Plant * ed->m_sois.por[il] * ed->m_sois.dz[il]
							* 1000.0;
				else
					Plant_m = Plant * ed->d_soid.alllwc[il] * ed->m_sois.dz[il]
							* 1000.0;
			}
			...
			V[il] = s[il] * ed->d_sois.ch4[il] + s[il] * dt * SS;
		...
		for (il = 1; il < numsl; il++)
			ed->d_sois.ch4[il] = V[il];

		tmp_flux = diff[1] * (ed->d_sois.ch4[1] - ub) / ed->m_sois.dz[1];
		...
		tmp_flux = (ed->m_sois.por[1] - ed->d_soid.alllwc[1]
				- ed->d_soid.alliwc[1]);
		if (tmp_flux < 0.05)
			tmp_flux = 0.05;
