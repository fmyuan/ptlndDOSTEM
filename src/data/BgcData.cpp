...
void BgcData::endOfMonth(const int & currmind) {
	...
	for (int il = 0; il < MAX_SOI_LAY; il++) {
		...
		y_soid.kdl_m[il] += m_soid.kdl_m[il] / 12;
		y_soid.kdr_m[il] += m_soid.kdr_m[il] / 12;
		y_soid.kdn_m[il] += m_soid.kdn_m[il] / 12;
...
void BgcData::endOfYear(const double & cnsoil) {
	...
	if (baseline == 1) {
		if ((m_soid.reacsum + m_soid.noncsum) / cnsoil >= m_sois.orgn) {
			y_a2soi.ninput = ((m_soid.reacsum + m_soid.noncsum) / cnsoil)
					- m_sois.orgn;
		} else {
			y_soi2l.nlost = m_sois.orgn - ((m_soid.reacsum + m_soid.noncsum)
					/ cnsoil);
		}
