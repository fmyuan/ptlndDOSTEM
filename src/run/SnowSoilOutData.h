...
class SnowSoilOutData{
	public:
	 ...
	 // for daily
			...
			float infild[DYINY];
			float wtd[DYINY];
			float dch4flux[DYINY];
			float dhco2flux[DYINY];
			float daco2flux[DYINY];
			float ddfratio[DYINY];
			float dco2ch4[DYINY];
			float doxid[DYINY];
			float fsatd[DYINY];
			float qoverd[DYINY]; 
	...
	// for monthly
			...
			float watertab[MINY];
			float mch4flux[MINY];
			float mhco2flux[MINY];
			float maco2flux[MINY];
			float mdfratio[MINY];
			float mco2ch4[MINY];
			float moxid[MINY];
			float mossdz[MINY];
			...
			float kdl_m[MINY][MAX_OUT_SOI];
			float kdr_m[MINY][MAX_OUT_SOI];
			float kdn_m[MINY][MAX_OUT_SOI];
...
//2D to 1D for easy Java reading (mainly soil variables)
			...
			float kdl1D_m[MINY*MAX_OUT_SOI];
			float kdr1D_m[MINY*MAX_OUT_SOI];
			float kdn1D_m[MINY*MAX_OUT_SOI];
