...
//////////////////////////////////////////////////////////////////////////////
void Ground::updateDaily(const int & yrcnt, const int & year, const int & mind,
		const int & id, const double & tdrv2, const double & dayl,
		const int & dayofyear) {
	double tsurface;
	...
	if (fstfntl == NULL && lstfntl == NULL) { // there is no front
		if (frontl->isSoil()) {
			if ((tstate == 1 && tdrv1 > 0) || (tstate == -1 && tdrv1 < 0)
					|| tstate == 0) {//create front
				soil.stefan.updateFronts(tdrv1, frontl, backl, fstsoill,
						lstminl, mind);
		...
		if (fstfntl->indl == lstfntl->indl) {
			if (fstfntl->isSoil()) {
				...
				if (sl->fronts[0]->frzing == -1) {//thawing front
					...
					if (fstfntl->prevl != NULL) {
						if (fstfntl->prevl->isSoil() && fstfntl->prevl->frozen
								== -1) {
							SoilLayer* slp =
									dynamic_cast<SoilLayer*> (fstfntl->prevl);
							if (slp->getVolLiq() > sl->getVolWater()) {
								double provide = (slp->getVolLiq()
										- sl->getVolWater()) * slp->dz * 1000.;
								double receive = (sl->poro - sl->getVolWater())
										* sl->dz * 1000.;
								double delta = 0;
								if (provide > receive) {
									delta = receive;
			...
			if (lstfntl->isSoil()) {
				...
				if (lstfntl->prevl != NULL) {
					if (sl->fronts[0]->frzing == -1) {//thawing front
						//put some water of upper layer into this layer
						if (lstfntl->prevl->isSoil() && lstfntl->prevl->frozen
								== -1) {
							SoilLayer* slp =
									dynamic_cast<SoilLayer*> (lstfntl->prevl);
							if (slp->getVolLiq() > sl->getVolWater()) {
								double provide = (slp->getVolLiq()
										- sl->getVolWater()) * slp->dz * 1000.;
								double receive = (sl->poro - sl->getVolWater())
	...
	if (curyrcnt == 0) {
		ed->d_vegs.preLAI = 0.0;
		ed->d_vegs.currLAI = 0.0;
	}
			...
			if (currl->isSnow()) {
				if (currl->dz < snow.mindz[currl->indl]) {
					if (currl->nextl->isSnow()) {
						...
						currl->nextl->rho = currl->nextl->ice
								/ currl->nextl->dz;
						currl->nextl->age = currl->nextl->age / 2. + currl->age
								/ 2.;
				...
				if (currl->dz < snow.mindz[currl->indl]) {
					if (currl->nextl->isSnow()) {
						...
						if (currl->nextl->dz < currl->prevl->dz) {
							...
							currl->prevl->rho = currl->prevl->ice
									/ currl->prevl->dz;
						}
...
bool Ground::constructSnowLayers(const double & tdrv) {
	bool layerchanged = false;
	double dsmass = (ed->d_v2g.sthfl + ed->d_v2g.sdrip + (1
			- ed->y_vegd.vegfrac) * ed->d_a2l.snfl - ed->d_snw2a.sublim
			- ed->d_snwd.melt); // dsmass is the total snowfall during one timestep
...
void Ground::initSnowSoilLayers5Thickness() {
	...
	for (int il = soil.mineral.num - 1; il >= 0; il--) {
		MineralLayer* ml = new MineralLayer(soil.mineral.dza[il],
				soil.mineral.type[il], &soillu);
		insertFront(ml);
...
void Ground::initSnowSoilLayers5Restart(RestartData * resin) {
	...
	for (int il = soil.mineral.num - 1; il >= 0; il--) {
		MineralLayer* ml = new MineralLayer(soil.mineral.dza[il],
				soil.mineral.type[il], &soillu);//0 means deep organic
		insertFront(ml);
...
void Ground::updateShlwThickness() {
	...
	if (upthick || thin || !samerange) {
		...
		if (num > 0 && soil.peat.shlwnum == 0) { //remove this layer
			...
			SoilLayer* shlwsl = dynamic_cast<SoilLayer*> (fstshlwl);
			divideOneSoilLayerU2L(shlwsl, plnew1, soil.peat.shlwdza[0],
					soil.peat.shlwdza[1]);
			...
			SoilLayer* shlwsl = dynamic_cast<SoilLayer*> (fstshlwl);
			divideOneSoilLayerU2L(shlwsl, plnew2, soil.peat.shlwdza[0]
					+ soil.peat.shlwdza[1], soil.peat.shlwdza[2]);
...
void Ground::divideOneSoilLayerU2L(SoilLayer*usl, SoilLayer* lsl,
		const double & updz, const double & lwdz) {
	...
	if (usl->nextl == NULL) {
		...
		double ntem = usl->nextl->ch4;
		double gradient = (usl->ch4 - ntem) / (-usl->dz - lsl->dz);
		lsl->ch4 = updz * gradient + usl->ch4;
	}

	if (usl->fronts.size() >= 1) {
		for (unsigned int i = 0; i < usl->fronts.size(); i++) {
			if (usl->fronts[i]->dz >= updz) {
				lsl->addOneFront5Bot(usl->fronts[i]->dz - updz,
						usl->fronts[i]->frzing);
			}
		}
...
void Ground::combineTwoSoilLayersL2U(SoilLayer* lsl, SoilLayer* usl) {
	...
	if (lsl->fronts.size() > 0) {
		for (unsigned int i = 0; i < lsl->fronts.size(); i++) {
			...
			usl->addOneFront5Bot(lsl->fronts[i]->dz + usl->dz,
					lsl->fronts[i]->frzing);
		}
	}
...
void Ground::setBgcData(Soil_Bgc * sslp) {
	ssl = sslp;
}
