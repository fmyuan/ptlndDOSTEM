...
void RestartOutputer::init(string& outputdir,string& stage, const int & numprocs, const int & myid){
	...
	kdfibV_m           =restartFile->add_var("KDFIB_m", ncDouble,chtD);
	kdhumV_m           =restartFile->add_var("KDHUM_m", ncDouble,chtD);
	kdminV_m           =restartFile->add_var("KDMIN_m", ncDouble,chtD);
	kdslowV_m        =restartFile->add_var("KDSLOW_m", ncDouble,chtD);
	RPcV        =restartFile->add_var("RP", ncDouble,chtD);
	TVEGcV        =restartFile->add_var("TVEG", ncDouble,chtD);
...
void RestartOutputer::outputVariables(const int & chtcount){
	...
	kdfibV_m->put_rec(&resod->kdfib_m, chtcount);
	kdhumV_m->put_rec(&resod->kdhum_m, chtcount);
	kdminV_m->put_rec(&resod->kdmin_m, chtcount);
	kdslowV_m->put_rec(&resod->kdslow_m, chtcount);
	RPcV->put_rec(&resod->rp, chtcount);
	TVEGcV->put_rec(&resod->tveg, chtcount);
...
int RestartOutputer::errorChecking(){
	...
	if (isnan(resod->kdfib_m) || isinf(resod->kdfib_m)) errcode = -1;
	if (isnan(resod->kdhum_m) || isinf(resod->kdhum_m)) errcode = -1;
	if (isnan(resod->kdmin_m) || isinf(resod->kdmin_m)) errcode = -1;
	if (isnan(resod->kdslow_m) || isinf(resod->kdslow_m)) errcode = -1;
	if (isnan(resod->rp) || isinf(resod->rp)) errcode = -1;
	if (isnan(resod->tveg) || isinf(resod->tveg)) errcode = -1;
