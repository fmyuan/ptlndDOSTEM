...
void Soil_Env::updateDailySurfFlux(Layer* frontl, const double & tsurf,
		const double & dayl) {
	...
	while (currl != NULL) {
		if (dzsum < totthick) {
			if (dzsum + currl->dz < totthick) {
				...
				liq += (currl->liq - currl->minliq) * (totthick - dzsum)
						/ totthick;
				dzsum = totthick;
...
double Soil_Env::getEvaporation(const double & tsurf, const double & dayl,
		const double &rad) {
...
void Soil_Env::initializeParameter(const int &drgtypep, const int &vegtypep) {
	...
	for (int i = 1; i < 10; i++) {
		envpar.frprod_frac[i] = envpar.frprod_frac[i - 1]
				+ chtlu->frprod_frac[vegtype][i];
	}
 ...
 * saturate first 3 layers (change to all organic layers and first layer mineral
 */
double Soil_Env::infilFrozen2(Layer* fstsoill, Layer *fstnoinfil,
	...
	while (currl->indl <= fstnoinfil->indl && totinfl > 0) {
		...
		if (currl->frozen == 0) {
			...
			if (totinfl > currl->maxliq - currl->liq && currl->maxliq
					- currl->liq > 0) {
...
double Soil_Env::getRunoffVIC(Layer* fstsoill, const double & rnth,
		const double & melt) {
	...
	if (inflow == 0) {
		...
		runoff = (inflow - top_max_moist + top_moist + top_max_moist * pow(
				basis, 1. + b_infilt));
...
double Soil_Env::getRunoff(Layer* fstsoill, const double & rnth,
		const double & melt, const double & frasat) {
...
double Soil_Env::update5Drainage(Layer* drainl, const double & fracsat,
		const double & wtd) {
	//CLM method
	double qdrainwet = 0;
	double qdraindry = 0;
	...
	while (currl != NULL) {
		if (currl->isSoil()) {
			...
			if (unffrac > 0) {
				...
				hk = sl->hksat * exp(-2 * (sl->z + sl->dz / 2)) * pow(
						(double) wetness1, (double) 2 * sl->bsw + 2);
				hksum += hk;
	...
	while (currl != NULL) {
		if (currl->isSoil()) {
			...
			if (layertop >= 1)
				break;
			if (!sl->isMoss()) {
				sl->rootfrac = getFineRootFrac(layertop, layerbot, mossthick)
						/ 100.;
			} else {
				sl->rootfrac = 0;
...
double Soil_Env::getFineRootFrac(const double & layertop,
		const double & layerbot, const double & mossthick) {
	double totfrfrac;
	...
	if (indtop == 0) {
		...
		sumfractop = (envpar.frprod_frac[indtop] - envpar.frprod_frac[indbot
				- 1]) / 10.0 * (topcm - indtop * 10.);
	}

	if (indbot == 0) {
		sumfracbot = envpar.frprod_frac[indbot] / 10.0 * botcm;
	} else {
		sumfracbot = (envpar.frprod_frac[indbot] - envpar.frprod_frac[indbot
				- 1]) / 10.0 * (botcm - indtop * 10.);
	}
...
void Soil_Env::retrieveDailyOutputs(Layer* fstsoill, Layer* fstminl,
		Layer* lstminl, Layer* backl) {
	...
	while (curr2 != NULL) {
		if (curr2->isSoil()) {
			...
			ed->d_soid.allaws[ind] = curr2->getVolLiq() / (curr2->poro
					- curr2->getVolIce());
				...
				for (unsigned int i = 0; i < sl->fronts.size(); i++) {
					if (sl->fronts[i]->frzing == 1) {
						...
						if (iff >= MAX_NUM_FNT) {
							...
							string msg =
									"The number of freezing fronts is more than MAX_OUT_FNT ";
							char* msgc = const_cast<char*> (msg.c_str());
						...
						if (itf >= MAX_NUM_FNT) {
							string msg =
									"The number of Thawing fronts is more than MAX_OUT_FNT ";
							char* msgc = const_cast<char*> (msg.c_str());
