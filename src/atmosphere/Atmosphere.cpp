...
void Atmosphere::prepareMonthDrivingData() {
	...
	for (int im = 0; im < 12; im++) {
		...
		for (int iy = MAX_ATM_DRV_YR - MAX_ATM_NOM_YR; iy < MAX_ATM_DRV_YR; iy++) { //Yuan: average over  the last 30 yrs atm data
...
void Atmosphere::prepareDayDrivingData() {
	...
	for (int im = 0; im < 12; im++) {
		...
		autil.updateDailyDriver(tad1, pvalt, cvalt, nvalt, dinmprev, dinmcurr,
				dinmnext);
		autil.updateDailyDriver(vapd1, pvalv, cvalv, nvalv, dinmprev, dinmcurr,
				dinmnext);
		autil.updateDailyPrec(0, im, precd1, dinmcurr, cvalt, cvalp, 0);

		for (int id = 0; id < 31; id++) {
			eq_ta_d[im][id] = tad1[id];
			eq_vap_d[im][id] = vapd1[id];
			precsplt(eq_ta_d[im][id], precd1[id], eq_snow_d[im][id],
					eq_rain_d[im][id]);
			...
			eq_abshd_d[im][id] = getAbsHumDeficit(eq_svp_d[im][id],
					eq_vap_d[im][id], tad1[id]);
		}
	...
	for (int im = 0; im < 12; im++) {
		...
		for (int id = 0; id < 31; id++) {
			...
			precsplt(ca_ta_d[im][id], precd1[id], ca_snow_d[im][id],
					ca_rain_d[im][id]);
			...
			ca_abshd_d[im][id] = getAbsHumDeficit(ca_svp_d[im][id],
					ca_vap_d[im][id] * 0.1, tad1[id]);
	...
	for (int iy = 0; iy < MAX_ATM_DRV_YR; iy++) {
		for (int im = 0; im < 12; im++) {
			...
			autil.updateDailyDriver(tad1, pvalt, cvalt, nvalt, dinmprev,
					dinmcurr, dinmnext);
			autil.updateDailyDriver(vapd1, pvalv, cvalv, nvalv, dinmprev,
					dinmcurr, dinmnext);
#ifdef M_PRECIP
			if (iy >= 104 && iy <= 110)
				autil.updateDailyPrec(iy, im, precd1, dinmcurr, cvalt, cvalp, 1);
			else
				autil.updateDailyPrec(iy, im, precd1, dinmcurr, cvalt, cvalp, 0);
#endif
#ifndef M_PRECIP
			autil.updateDailyPrec(0, im, precd1, dinmcurr, cvalt, cvalp, 0);
#endif
			...
			for (int id = 0; id < 31; id++) {
				...
				precsplt(ta_d[iy][im][id], precd1[id], snow_d[iy][im][id],
						rain_d[iy][im][id]);

				rhoa_d[iy][im][id] = getDensity(tad1[id]);
				svp_d[iy][im][id] = getSatVP(tad1[id]);
				vpd_d[iy][im][id]
						= getVPD(svp_d[iy][im][id], vap_d[iy][im][id]);
				dersvp_d[iy][im][id] = getDerSVP(tad1[id], svp_d[iy][im][id]);
				abshd_d[iy][im][id] = getAbsHumDeficit(svp_d[iy][im][id],
						vap_d[iy][im][id], tad1[id]);
			} //id
		}//im
...
void Atmosphere::beginOfMonth(const int & curyrcnt, const int& currmind,
		const int& dinmcurr, const bool & normal, const bool & changeclm,
		const bool & changeco2) {

	if (normal) { //using normalized weather data of first 30 yrs (modified in inc/timeconst.h)

		if (changeco2) {
			ed->m_atms.co2 = ed->rd->co2[curyrcnt % MAX_CO2_DRV_YR]; //Yuan: this reuse CO2 data sets
		} else {
		...
		int yrind;
		if (changeclm) {
			yrind = curyrcnt % MAX_ATM_DRV_YR; //Yuan: this will reuse the whole atm data set
		} else {
			yrind = curyrcnt % MAX_ATM_NOM_YR; //Yuan: this will reuse the atm data of first 30 yrs
		}
...
void Atmosphere::updateDailyEnviron(const int &yrcnt, const int & mid,
		const int & dayid, const bool & normal, const bool & changeclm) {
	...
	if (normal) { //using normalized climatic driving
		...
		int yrind;
		if (changeclm) {
			yrind = yrcnt % MAX_ATM_DRV_YR; //Yuan: this will reuse the whole atm data set
		} else {
			yrind = yrcnt % MAX_ATM_NOM_YR; //Yuan: this will reuse the atm data of first 30 yrs
		}
...
void Atmosphere::precsplt(const float & tair, const float & prec, float & snfl,
		float & rnfl) {
	/* *************************************************************
...
float Atmosphere::getGIRR(const float &lat, const int& dinm) {
	...
	for (day = 0; day < dinm; day++) {
		...
		for (hour = 0; hour < 24; hour++) {
			...
			sinbeta = sin(lambda) * sin(sig) + cos(lambda) * cos(sig)
					* cos(eta);
			sotd = 1 - (0.016729 * cos(0.9856 * (yrsumday - 4.0) * pi / 180.0));
...
float Atmosphere::getAbsHumDeficit(const float & svp, const float &vp,
		const float & ta) {
	...
	ashd = fabs((Mw / R) * ((svp * 1000. / (273.2 + dewpnt)) - (vp * 1000
			/ (273.2 + ta))));
	return ashd;
}
;
