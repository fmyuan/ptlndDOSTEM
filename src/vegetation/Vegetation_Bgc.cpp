...
void Vegetation_Bgc::initializeParameter(const int & drgtypep,
		const int & vegtypep) {
	...
	if (nfeed == 1) {
		...
		if (inprodcn > cneven) {//cneven is corresponding to Pcn in ATMcGuire71992a P106
			...
			del_soi2v.nuptake = del_soi2v.innuptake * (inprodcn - bgcpar.cnmin)
					* (inprodcn - 2 * cneven + bgcpar.cnmin);
			del_soi2v.nuptake /= ((inprodcn - bgcpar.cnmin) * (inprodcn - 2
					* cneven + bgcpar.cnmin)) - pow(inprodcn - cneven, 2.0);
			if (del_soi2v.nuptake < 0.0) {
				del_soi2v.nuptake = 0.0;
			}
			...
			if ((tmp_vegs.ston + del_soi2v.nuptake - del_soi2v.suptake
				...
				del_soi2v.luptake = (bgcpar.labncon * (tmp_vegs.strn
						...
						- del_v2v.nresorb + del_v2v.nmobil)) - (tmp_vegs.ston
						+ del_v2v.nresorb - del_v2v.nmobil);
				if (del_soi2v.luptake < 0.0) {
					del_soi2v.luptake = 0.0;
				}
...
double Vegetation_Bgc::getUnnormleaf(double &prveetmx, const double & eet,
...
double Vegetation_Bgc::getGV(const double & eet, const double & pet) {
	...
	if (eet / pet <= 0.1) {
		gv = (-10.0 * pow((eet / pet), 2.0)) + (2.9 * (eet / pet));
		if (gv < 0.0) {
			gv = 0.0;
		}
	} else {
...
double Vegetation_Bgc::getGPP(const double &co2, const double & par,
		const double &leaf, const double & foliage, const double &ftemp,
		const double & gv) {
	// origianlly the 
	double ci = co2 * gv;
	...
	if (tair <= tmin || tair >= tmax) {
		...
		if (tair >= tgppopt && tair <= toptmax) {
			...
			if (tair > tmin && tair < tgppopt) {
				ftemp = (tair - tmin) * (tair - tmax) / ((tair - tmin) * (tair
						- tmax) - pow((tair - tgppopt), 2.0));
			} else {
				ftemp = (tair - tmin) * (tair - tmax) / ((tair - tmin) * (tair
						- tmax) - pow((tair - toptmax), 2.0));
			}
		}
	}
