...
void Stefan::updateFronts(const double & tdrv, Layer *frontl, Layer *backl,
	...
	if (lstfntl != NULL) {
		...
		if (slstfntl->fronts[numfnt - 1]->frzing == 1 && botdrvl->tem < 0) {
			botdrvl = NULL;
		} else if (slstfntl->fronts[numfnt - 1]->frzing == -1 && botdrvl->tem
				> 0) {
			botdrvl = NULL;
		}
	}
...
void Stefan::processSoilLayer5Bot(const int &frozenstate, double const & tkres,
		double const & tkfront, double & dse, double & sumresblw,
		const double & tdrv, SoilLayer* sl, Layer *frontl, Layer *backl) {
	...
	if (numfnt > 0) {
		...
		if ((sl->frozen == 1 && tdrv > 0) or (sl->frozen == -1 && tdrv < 0)) {
			if (sl->nextl->isRock()) {//to exclude of the case that rock layer change from very small minus temperature
				...
				string msg =
						"there should be no new front created from bottom 2";
				char* msgc = const_cast<char*> (msg.c_str());
				throw Exception(msgc, I_BOTTOM_NEW_FRONT);
			}
		}
...
void Stefan::updateTemps(const double & tdrv, Layer *frontl, Layer *backl,
	...
	if (fstfntl == NULL && lstfntl == NULL) {
		...
		itsumall = itsumabv + itsumblw;
		// for layers between fstfntl and lstfntl
		/*currl=fstfntl->nextl;
		 while(currl!=NULL){
		 currl->tem= 0.;
		 currl=currl->nextl;
...
#ifndef RAISED
	if ((year == 2005 && doy >= 171) || year == 2006 || year == 2007 || (year
					== 2008 && doy <= 213) || year ==2009 || year == 2010 || (year == 2011 && doy >=292)) {
#endif
#ifdef RAISED
		if ((year == 2005 && doy >= 193) || year == 2006 || year == 2007 && doy
				<= 274 || (year == 2008 && doy >= 98) || year ==2009 || year == 2010 || (year == 2011 && doy >=292)) {
#endif
			...
			while (currl != NULL) {
				if (currl->isSoil()) {
					...
					if (tmp1 >= 0.5) {
						if (year == 2005)
#ifndef RAISED
						temp50 = temp2005[doy - 171][4];
#endif
#ifdef RAISED
						temp50 = temp2005[doy - 193][4];
#endif
						if (year == 2006)
						temp50 = temp2006[doy][4];
						if (year == 2007)
						temp50 = temp2007[doy][4];
						if (year == 2008)
#ifndef RAISED
						temp50 = temp2008[doy][4];
#endif
#ifdef RAISED
						temp50 = temp2008[doy - 98][4];
#endif
...
#ifndef RAISED
						if (year == 2005) {
							deptharray[0] = temp2005[doy - 171][0];
							deptharray[1] = temp2005[doy - 171][1];
							deptharray[2] = temp2005[doy - 171][2];
							deptharray[3] = temp2005[doy - 171][3];
							deptharray[4] = temp2005[doy - 171][4];
						}
#endif
#ifdef RAISED
						if (year == 2005) {
							deptharray[0] = temp2005[doy - 193][0];
							deptharray[1] = temp2005[doy - 193][1];
							deptharray[2] = temp2005[doy - 193][2];
							deptharray[3] = temp2005[doy - 193][3];
							deptharray[4] = temp2005[doy - 193][4];
						}
#endif
						if (year == 2006) {
							deptharray[0] = temp2006[doy][0];
							deptharray[1] = temp2006[doy][1];
							deptharray[2] = temp2006[doy][2];
							deptharray[3] = temp2006[doy][3];
							deptharray[4] = temp2006[doy][4];
						}
						if (year == 2007) {
							deptharray[0] = temp2007[doy][0];
							deptharray[1] = temp2007[doy][1];
							deptharray[2] = temp2007[doy][2];
							deptharray[3] = temp2007[doy][3];
							deptharray[4] = temp2007[doy][4];
						}
#ifndef RAISED
						if (year == 2008) {
							deptharray[0] = temp2008[doy][0];
							deptharray[1] = temp2008[doy][1];
							deptharray[2] = temp2008[doy][2];
							deptharray[3] = temp2008[doy][3];
							deptharray[4] = temp2008[doy][4];
						}
#endif

#ifdef RAISED
						if (year == 2008) {
							deptharray[0] = temp2008[doy - 98][0];
							deptharray[1] = temp2008[doy - 98][1];
							deptharray[2] = temp2008[doy - 98][2];
							deptharray[3] = temp2008[doy - 98][3];
							deptharray[4] = temp2008[doy - 98][4];
						}

#endif
						if (year == 2009) {
							deptharray[0] = temp2009[doy][0];
							deptharray[1] = temp2009[doy][1];
							deptharray[2] = temp2009[doy][2];
							deptharray[3] = temp2009[doy][3];
							deptharray[4] = temp2009[doy][4];
						}
						if (year == 2010) {
						...
						inter_temp = linear_interp(soildepth, deptharray, 0, 4,
								tmp1);
...
void Stefan::iterate(const int &startind, const int &endind,
	...
	while (tmld < 1) {
		...
		int st = updateOneTimeStep(startind, endind, lstlaybot, fstlaytop,
				frontl);
...
int Stefan::updateOneTimeStep(const int startind, const int & endind,
		const bool & lstlaybot, const bool & fstlaytop, Layer *frontl) {
	int status = -1;
	int is;
...
void Stefan::processAboveLayers(Layer* frontl, Layer *backl, Layer*fstfntl,
...
void Stefan::processBetweenLayers(Layer* frontl, Layer *backl, Layer*fstfntl,
		Layer*lstfntl, const double & tdrv) {
	...
	/*
	 ...
	 s[ind] = 0.;
	 e[ind]= t[ind];
	 startind =fstfntl->indl; //always zero
	 endind= ind; //
...
void Stefan::updateUpperTemps5Front(Layer* fstfntl, const double & beforedse,
				...
				if (i == 0) {
					if (currl->prevl != NULL) {
						if (currl->prevl->isSoil()) {
							SoilLayer* psl =
									dynamic_cast<SoilLayer*> (currl->prevl);
							if (psl->fronts.size() == 0) {
								if (psl->frozen != sl->fronts[i]->frzing) {
									string msg =
											"front should have same attribute as previous layer frozen state";
									char* msgc =
...
double linear_interp(const double *X, const double *Y, const int begin,
	...
	j = bisection(X, x, begin, end);
	A1 = (X[j + 1] - x) / (X[j + 1] - X[j]);
	A2 = 1.0 - A1;
	r = (A1 * Y[j] + A2 * Y[j + 1]);
...
int bisection(const double *a, const double x, const int low, const int high) {
	...
	if (low > high) {
		//printf("Error: Function 'bisection' returned -1 becasue low > high.\n");
		exit(1);
	}
	if (x < a[low])
		return low;
	...
	if (high - low == 1)
		return low;
	if (x < a[mid])
		return bisection(a, x, low, mid);
	else
		return bisection(a, x, mid, high);
