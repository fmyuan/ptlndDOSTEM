...
void SoilclmInputer::init(string & outdir){
	...
	chtD = sclmFile->get_dim("CHTID");
 	if(!chtD->is_valid()){
 		throw Exception("Cohort Dimension is no Valid in SoilclmInputer::init", I_NCDIM_NOT_EXIST);
 	}
	...
	mlayerD = sclmFile->get_dim("MINLAYER");
 	...
 	if(chtidV==NULL){
 	   throw Exception("Cannot get chtidv in SoilclmInputer::init ", I_NCVAR_NOT_EXIST);
 	}
...
void SoilclmInputer::getChtId(int &chtid, const int &recid){
	...
	if(!nb1){
	 throw Exception("problem in reading chtid in SoilclmInputer",  I_NCVAR_GET_ERROR);
	}
}
...
void SoilclmInputer::getSoilClimate(SoilClm * sclm, const int & recid){
	...
	tsV->get(&sclm->ts[0][0], 12, MAX_SOI_LAY);
  	...
  	ch4V->set_cur(recid);
	ch4V->get(&sclm->ch4[0][0], 12, MAX_SOI_LAY);
