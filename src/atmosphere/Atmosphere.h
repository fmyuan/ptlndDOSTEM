...
public:
	...
	void updateDailyEnviron(const int &yrcnt, const int & mid,
	...
	void beginOfMonth(const int & curyrcnt, const int& currmind,
...
private:
	...
	float
			getAbsHumDeficit(const float & svp, const float &vp,
					const float & ta);
	float wetdays;

	float yrsumday;
	void precsplt(const float & tair, const float & prec, float & snfl,
			float & rnfl);
